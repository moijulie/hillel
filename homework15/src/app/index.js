...
class App {
    ...
    reneder() {
        const listItems = this.list.querySelectorAll('.list__item');
        listItems.forEach((item) => {
            const fields = item.querySelectorAll('[data-field]')
            fields.forEach((span) => {
                const fieldValue = span.dataset.field;
                if(fieldValue in USER_DATA) {
                    const newContent = document.createTextNode(USER_DATA[fieldValue]());
                    item.replaceChild(newContent, span);
                }
            })
...
class Timestamp {
    ...
    toString() {
        const date = new Date();
        return date.getFullYear() + '-' + DATE.getMonthStr(date) + '-' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
    }
}
